...
impl WorkspaceRow {
    fn into_domain(self) -> Result<Workspace, RepositoryError> {
        let id = uuid::Uuid::parse_str(&self.id)
            .map_err(|e| decode_err(format!("invalid workspace id '{}': {e}", self.id)))?;
        let created_by = uuid::Uuid::parse_str(&self.created_by_user_id).map_err(|e| {
            decode_err(format!(
                "invalid workspace created_by_user_id '{}': {e}",
...
struct WorkspaceMembershipRow {
    ...
    role: String,
    status: String,
    can_manage_members: i32,
    can_manage_nips: i32,
    can_manage_credentials: i32,
    created_at: String,
    updated_at: String,
}
...
impl WorkspaceMembershipRow {
    fn into_domain(self) -> Result<WorkspaceMembership, RepositoryError> {
        let workspace_id = uuid::Uuid::parse_str(&self.workspace_id).map_err(|e| {
            decode_err(format!("invalid workspace_id '{}': {e}", self.workspace_id))
        })?;
        ...
        Ok(WorkspaceMembership {
            ...
            can_manage_members: self.can_manage_members != 0,
            can_manage_nips: self.can_manage_nips != 0,
            can_manage_credentials: self.can_manage_credentials != 0,
            created_at: parse_datetime(&self.created_at, "created_at")?,
            updated_at: parse_datetime(&self.updated_at, "updated_at")?,
        })
    }
...
struct WorkspaceSummaryRow {
    ...
    workspace_created_by_user_id: String,
    workspace_created_at: String,
    workspace_updated_at: String,
    membership_user_id: String,
    membership_role: String,
    membership_status: String,
    membership_can_manage_members: i32,
    membership_can_manage_nips: i32,
    membership_can_manage_credentials: i32,
...
impl WorkspaceSummaryRow {
    fn into_domain(self) -> Result<WorkspaceSummary, RepositoryError> {
        let workspace_id = uuid::Uuid::parse_str(&self.workspace_id).map_err(|e| {
            decode_err(format!("invalid workspace id '{}': {e}", self.workspace_id))
        })?;
        let created_by =
            uuid::Uuid::parse_str(&self.workspace_created_by_user_id).map_err(|e| {
                decode_err(format!(
                    "invalid workspace created_by_user_id '{}': {e}",
                    self.workspace_created_by_user_id
                ))
            })?;
        let membership_user_id = uuid::Uuid::parse_str(&self.membership_user_id).map_err(|e| {
            decode_err(format!(
                "invalid membership user_id '{}': {e}",
                self.membership_user_id
            ))
        })?;

        Ok(WorkspaceSummary {
            workspace: Workspace {
                id: WorkspaceId::from_uuid(workspace_id),
                slug: self.workspace_slug,
                display_name: self.workspace_display_name,
                created_by_user_id: UserId::from_uuid(created_by),
                created_at: parse_datetime(&self.workspace_created_at, "workspace_created_at")?,
                updated_at: parse_datetime(&self.workspace_updated_at, "workspace_updated_at")?,
            },
            membership: WorkspaceMembership {
                workspace_id: WorkspaceId::from_uuid(workspace_id),
                user_id: UserId::from_uuid(membership_user_id),
                role: self.membership_role.parse().map_err(decode_err)?,
                status: self.membership_status.parse().map_err(decode_err)?,
                can_manage_members: self.membership_can_manage_members != 0,
                can_manage_nips: self.membership_can_manage_nips != 0,
                can_manage_credentials: self.membership_can_manage_credentials != 0,
                created_at: parse_datetime(&self.membership_created_at, "membership_created_at")?,
                updated_at: parse_datetime(&self.membership_updated_at, "membership_updated_at")?,
            },
        })
    }
}
...
impl WorkspaceInviteRow {
    fn into_domain(self) -> Result<WorkspaceInvite, RepositoryError> {
        let id = uuid::Uuid::parse_str(&self.id)
            .map_err(|e| decode_err(format!("invalid invite id '{}': {e}", self.id)))?;
        let workspace_id = uuid::Uuid::parse_str(&self.workspace_id).map_err(|e| {
            decode_err(format!("invalid workspace id '{}': {e}", self.workspace_id))
        })?;
        let created_by = uuid::Uuid::parse_str(&self.created_by_user_id).map_err(|e| {
            decode_err(format!(
                "invalid invite created_by_user_id '{}': {e}",
                self.created_by_user_id
            ))
        })?;

        Ok(WorkspaceInvite {
            id: WorkspaceInviteId::from_uuid(id),
            workspace_id: WorkspaceId::from_uuid(workspace_id),
            email: self.email,
            role: self.role.parse().map_err(decode_err)?,
    ...
    E: SqliteExecutor<'e> + Copy,
{
    ...
    .bind(user_id.to_string())
    .bind(role.to_string())
    .bind(i32::from(role.can_manage_members()))
    .bind(i32::from(role.can_manage_nips()))
    .bind(i32::from(role.can_manage_credentials()))
    .bind(&now)
    .bind(&now)
    .execute(exec)
    .await?;
    ...
    E: SqliteExecutor<'e> + Copy,
{
    sqlx::query(
        ...
        ON CONFLICT(workspace_id, nip_account_id) DO UPDATE SET
