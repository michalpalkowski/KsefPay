...
impl TokenRow {
    fn into_domain(self) -> Result<StoredTokenPair, RepositoryError> {
        Ok(StoredTokenPair {
            id: self.id,
            nip: Nip::parse(&self.nip).map_err(|e| {
                RepositoryError::Database(sqlx::Error::Decode(
                    format!("invalid NIP in token row: {e}").into(),
                ))
            })?,
...
impl SessionRow {
    fn into_domain(self) -> Result<StoredSession, RepositoryError> {
        Ok(StoredSession {
            id: self.id,
            session_reference: SessionReference::new(self.session_reference),
            nip: Nip::parse(&self.nip).map_err(|e| {
                RepositoryError::Database(sqlx::Error::Decode(
                    format!("invalid NIP in session row: {e}").into(),
                ))
            })?,
            environment: parse_env(&self.environment)?,
            created_at: self.created_at,
