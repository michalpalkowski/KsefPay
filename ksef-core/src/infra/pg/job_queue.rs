...
impl JobRow {
    fn try_into_domain(self) -> Result<Job, QueueError> {
...
impl JobQueue for PgJobQueue {
    ...
    /// Atomically dequeue one pending job using `FOR UPDATE SKIP LOCKED`.
    ...
    async fn fail(&self, id: &JobId, error: &str) -> Result<(), QueueError> {
        // Increment attempts. If max reached, move to dead_letter.
    ...
    async fn list_pending(&self) -> Result<Vec<Job>, QueueError> {
        let rows: Vec<JobRow> = sqlx::query_as(
            "SELECT id, job_type, payload, status, attempts, max_attempts, last_error, created_at FROM jobs WHERE status = 'pending' ORDER BY scheduled_at ASC",
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    async fn list_dead_letter(&self) -> Result<Vec<Job>, QueueError> {
        let rows: Vec<JobRow> = sqlx::query_as(
            "SELECT id, job_type, payload, status, attempts, max_attempts, last_error, created_at FROM jobs WHERE status = 'dead_letter' ORDER BY created_at DESC",
        )
        .fetch_all(&self.pool)
        .await?;
